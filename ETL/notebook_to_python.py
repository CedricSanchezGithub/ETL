...
MYSQL_DRIVER_PATH = os.path.join(
    BASE_DIR, "installation", "mysql-connector-j-9.1.0.jar"
)
...
def main_function():
    ...
    folder_all_animals = [
        d
        for d in os.listdir(TRAIN_IMAGE_DIR)
        ...
        if row["Coeff"] < 4:
            ...
            for img_file in image_files:
                ...
                for i in range(coeff):
                    ...
                    cv2.imwrite(
                        new_img_path, cv2.cvtColor(augmented, cv2.COLOR_RGB2BGR)
                    )
    ...
    for folder in df_all_animals["Nom du dossier"]:
        ...
        image_data_new.append(
            [folder, num_images_train, avg_width_train, avg_height_train]
        )
    ...
    df_dict = {}
    for dossier in (
        df_merged_spark.select("Nom du dossier")
        .distinct()
        ...
        if os.path.exists(folder_path):
            df_images = spark.createDataFrame(
                [
                    (dossier, os.path.join(dossier, img))
                    for img in os.listdir(folder_path)
                ],
                ["Nom du dossier", "Chemin Relatif"],
            )
            df_dict[dossier] = df_images.join(
                df_merged_spark, on="Nom du dossier", how="left"
            )
    ...
    if df_dict:
        ...
        df_final.write.mode("overwrite").parquet(
            os.path.join(PARQUET_OUTPUT_DIR, "all_images")
        )
        df_final.coalesce(1).write.mode("overwrite").csv(
            os.path.join(CSV_OUTPUT_DIR, "all_images"), header=True
        )
        print(
            f" Fusion complète ! Le DataFrame final contient {df_final.count()} images."
        )
    else:
    ...
    if df_existing.count() == 0:
        ...
        df_facts = (
            df_facts.withColumnRenamed("Espèce anglais", "nom_en")
    ...
    df_final = df_final.join(
        df_id_espece, df_final["Espèce français"] == df_id_espece["nom_fr"], "left"
    )
    ...
    df_final = df_final.withColumnRenamed("Chemin Relatif", "image").withColumnRenamed(
        "state", "id_etat"
    )
